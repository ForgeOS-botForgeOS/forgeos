...
// What the food IS is the half of the job a photo is worst at — a home-cooked
// curry and a takeaway one look identical and are not. So when the user tells
// us, we believe them for identification and let the model do what it can
// actually see: how much is on the plate.
const MAX_NOTE_CHARS = 200;
function foodPrompt(note) {
  const hint = String(note || '').replace(/[{}\r\n]+/g, ' ').replace(/\s+/g, ' ').trim().slice(0, MAX_NOTE_CHARS);
  if (!hint) return FOOD_PROMPT;
  return (
    `The person who took this photo says the food is: "${hint}". Trust that ` +
    'description for WHAT the foods are (use their words in the item names, and ' +
    'use any weight they state); use the photo for the portion sizes of anything ' +
    'they did not weigh, and for anything visible they did not mention. ' +
    FOOD_PROMPT
  );
}
...
export default {
  async fetch(request, env) {
    ...
    try {
      const { image, mode, source, note } = await request.json();
      ...
      const ai = await env.AI.run('@cf/llava-hf/llava-1.5-7b-hf', {
        prompt: isCardio ? cardioPrompt(source) : foodPrompt(note),
        image: [...bytes],
        max_tokens: 512,
        temperature: 0.1,
      });
      ...
      let rawItems = Array.isArray(data?.items) && data.items.length ? data.items : null;
      // The model read nothing usable. If the user told us what the food is,
      // that description is still worth more than a flat "Meal" — price it as
      // one default portion and let them fix the grams.
      if (!rawItems && note) rawItems = [{ name: String(note).slice(0, MAX_NOTE_CHARS), grams: 300 }];
      if (!rawItems) {
