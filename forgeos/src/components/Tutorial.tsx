...
import { Home, Dumbbell, Apple, Users, Trophy, User, Flame, X } from 'lucide-react';
import { Button } from './ui';

const SEEN = 'forge-tutorial-seen';

// Fire from anywhere (e.g. Profile) to replay the tour.
export function openTutorial() {
  window.dispatchEvent(new Event('forge:tutorial'));
}
    ...
    <Overlay>
      ...
      <div className="text-center space-y-3">
        ...
        <div className="flex gap-2 pt-1">
          ...
          <Button className="flex-1 justify-center" onClick={() => (last ? finish() : setI(i + 1))}>{last ? 'Start training 🔥' : 'Next'}</Button>
        </div>
      </div>
    </Overlay>
  );
