...
function pickFocus(r: Omit<WeeklyReview, 'focus'>): string {
  if (r.readinessTrend === 'down') return 'Recovery is trending down — keep one or two sessions deliberately light this week.';
  if (r.volumeDeltaPct != null && r.volumeDeltaPct <= VOLUME_DROP_PCT) return 'Volume dipped — lock in your planned sessions before adding anything new.';
  if (r.prCount === 0 && (r.volumeDeltaPct ?? 0) > 0) return "Volume is climbing and no PR fell — pick one lift and go chase it this week.";
  if (r.prCount > 0) return 'PRs are dropping — ride the wave, add 2.5 kg where last week felt easy.';
  return 'Keep the rhythm: same training days, +2.5 kg wherever last week felt easy.';
}
 ...
 */
export function buildWeeklyReview(history: Workout[], prs: PR[], days: HealthDay[], now: number): WeeklyReview | null {
  ...
  const base = {
    weekLabel: label(new Date(from), new Date(thisMonday - 86_400_000)),
  ...
  return { ...base, focus: pickFocus(base) };
}
