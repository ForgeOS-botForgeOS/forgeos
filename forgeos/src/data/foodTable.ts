// Per-100 g (or 100 ml) macros for generic foods: [kcal, protein, carbs, fat, sugar].
//
// This is the same curated table the vision Worker uses (`worker/src/index.js`)
// — kept here as well so a typed food description resolves instantly, offline,
// with no round trip. Generic whole foods live here on purpose: crowd-sourced
// databases are full of branded near-misses for words like "rice" or "chicken",
// and a curated number beats a confident wrong one. Keep the two in sync when
// adding foods; the Worker copy is the one the photo path uses.
//
// Sources: USDA FoodData Central / manufacturer averages, rounded.

/** [kcal, protein g, carbs g, fat g, sugar g] per 100 g. */
export type Per100g = readonly [number, number, number, number, number];

export const FOOD_TABLE: Readonly<Record<string, Per100g>> = {
